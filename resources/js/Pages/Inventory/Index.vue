...
let inventorysUrl = computed(() => {
    let url = new URL(route("inventories.index"));
...
watch(
    () => inventorysUrl.value,
    (updatedinventorysUrl) => {
        router.visit(updatedinventorysUrl, {
            preserveScroll: true,
            preserveState: true,
            replace: true,
        });
...
const deleteInventory = (inventoryId) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            deleteForm.delete(route("inventories.destroy", inventoryId), {
                preserveScroll: true,
            });
            Swal.fire({
                title: "Deleted!",
        ...
        <DefaultLayout>
            <div class="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
               ...
               dark:border-strokedark sm:grid-cols-4 md:px-6 2xl:px-7.5">
                ...
                <!-- Table Rows -->
                <div v-for="inventorie in inventories.data" :key="inventorie.id" class="grid grid-cols-3 border-t border-stroke 
                 py-4.5 px-4 dark:border-strokedark sm:grid-cols-4 md:px-6 2xl:px-7.5">
                    <div class="col-span-2 flex items-center">
                        <p class="text-sm font-medium text-black dark:text-white">{{ inventorie.fecha }}</p>
                    </div>
                    <!--  -->
                    <div class="col-span-1 flex items-center">
            ...
            <Pagination :data="inventories" :updatedPageNumber="updatedPageNumber" />
        </DefaultLayout>
    </AuthenticatedLayout>
</template>